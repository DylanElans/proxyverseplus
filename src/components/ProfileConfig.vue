...
import { FieldRule, Notification, Message } from "@arco-design/web-vue";
...
try {
  const current = toRaw(profileConfig);

  const allProfiles = await listProfiles();
  const colorConflict = Object.values(allProfiles).some(
    (p) => p.profileID !== current.profileID && p.color === current.color
  );

  if (colorConflict) {
    Message.error("当前颜色已被其它代理使用，请更换一个颜色");
    return;
  }
